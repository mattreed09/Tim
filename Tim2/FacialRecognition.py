...
def FacialRecognition(cameraCount, IsTheCarMoving, DelayUntilMoveSeconds):
    ...
    cascPath = "C:/temp/haarcascade_frontalface_default.xml"
    faceCascade = cv2.CascadeClassifier(cascPath)
    priors,frame = [[0,0,0,0],[0,0,0,0]]
    i, start, end=[0,0,0]
    lastCamera ,lastFace , lastImage,isTimerGoing= [[0],[0],[0],False]
    
    IsThereAFace= [not IsTheCarMoving,not IsTheCarMoving,not IsTheCarMoving,not IsTheCarMoving]
    while True:
        i = (i + 1) % cameraCount
